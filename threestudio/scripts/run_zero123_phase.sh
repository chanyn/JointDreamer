...
#    data.image_path=./load/cvpr24/${IMAGE_PREFIX}_rgba.png system.freq.guidance_eval=37 \
...
python launch.py --config configs/zero123.yaml --train --gpu $GPU_ID system.loggers.wandb.enable=false \
    data.image_path=./load/cvpr24/${IMAGE_PREFIX}_rgba.png system.freq.guidance_eval=37 \
    system.guidance.cond_elevation_deg=$ELEVATION \
    ${REST}
...
class SaverMixin:
    ...
    def save_rgba_image(
        ...
        alpha = 255 * np.ones((img.shape[0], img.shape[1]), dtype=img.dtype,)
        alpha = alpha * self.convert_data(mask)
        img = np.concatenate([img, alpha[..., np.newaxis].astype(np.uint8)], axis=2)

        nc = 4
        imgs = [img[..., start: start + nc] for start in range(0, img.shape[-1], nc)]
        imgs = [img_ for img_ in imgs]
        img = np.concatenate(imgs, axis=1)
    ...
    def save_torch_image(self, filename, img):
        from torchvision.utils import save_image
        save_image(img, self.get_save_path(filename))

...
class StableDiffusionGuidance(BaseObject):
    ...
    def compute_grad_sjc(
        ...
        Ds = zs - sigma * noise_pred # denoised latent
...
@threestudio.register("stable-diffusion-guidance2")
class StableDiffusionGuidance2(StableDiffusionGuidance):
    def __call__(
        self,
        rgb: Float[Tensor, "B H W C"],
        prompt_utils: PromptProcessorOutput,
        ...
        **kwargs,
        ...
        target = (latents - grad).detach()
        # d(loss)/d(latents) = latents - target = latents - (latents - grad) = grad
        loss_sds_ori = 0.5 * F.mse_loss(latents, target, reduction="sum") / batch_size

        guidance_out = {
            "loss_sds_ori": loss_sds_ori,
            "grad_sds_norm": grad.norm(),
            "min_step": self.min_step,
            "max_step": self.max_step,
        }

...
if __name__ == "__main__":
    ...
    sys.path.append("/home/dycpu3_8tssd/jch/threestudio-main")

    output_dir = "/home/dycpu3_8tssd/jch/exps/zero123-xl"
    cfg = load_config("configs/debugging/zero123.yaml")
    elev = 40.
    azim = 220.

    guidance = threestudio.find(cfg.system.guidance_type)(cfg.system.guidance)
    guidance.configure()
    cond_elev = torch.FloatTensor([elev]).to(guidance.device)
    cond_azim = torch.FloatTensor([azim]).to(guidance.device)
    cond_dist = torch.FloatTensor([3.8]).to(guidance.device)

    image = guidance.generate(guidance.rgb_256, cond_elev, cond_azim, cond_dist, guidance.c_crossattn, guidance.c_concat, ddim_steps=50)
    print(image.shape)
    os.makedirs("output_dir", exist_ok=True)
    cv2.imwrite(
        os.path.join(output_dir, f"elev{elev}_azim{azim}2.jpg"), image[0] * 255.0)
    print("Saved image.")

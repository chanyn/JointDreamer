...
@threestudio.register("flexicube-sdf-grid")
class FlexiCubeSDFGrid(BaseExplicitGeometry):
    @dataclass
    ...
    def configure(self) -> None:
        ...
        self.deform: Float[Tensor, "Nv 3"]
        self.weight: Float[Tensor, "Nf 21"]

        self.flexicube = kal.non_commercial.FlexiCubes(self.device) # initialize flexicube
        x_nx3, cube_fx8 = self.flexicube.construct_voxel_grid(self.cfg.voxel_grid_res)
        ...
        self.register_parameter(
            "sdf",
            nn.Parameter(
                torch.rand_like(x_nx3[:, 0]) - 0.1
            ),
        )
        self.register_parameter(
            "weight",
            nn.Parameter(
                torch.zeros((cube_fx8.shape[0], 21), dtype=torch.float)
            ),
        )
        self.register_parameter(
            "deform",
            nn.Parameter(
                torch.zeros_like(x_nx3)
            ),
        )
    ...
    def isosurface(self, return_L=False) -> Mesh:
        ...
        grid_verts = self.x_nx3 + (2 - 1e-8) / (self.cfg.voxel_grid_res * 2) * torch.tanh(self.deform)
        vertices, faces, L_dev = self.flexicube(
            grid_verts, self.sdf, self.cube_fx8, self.cfg.voxel_grid_res, beta=self.weight[:, :12], alpha=self.weight[:, 12:20],

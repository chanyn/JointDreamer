...
@threestudio.register("nerf-volume-renderer-temp")
class NeRFVolumeRendererTemp(NeRFVolumeRenderer):
    def forward(
        self,
        rays_o: Float[Tensor, "B H W 3"],
        rays_d: Float[Tensor, "B H W 3"],
        ...
        **kwargs
        ...
        if self.training:
            geo_out = self.geometry(
                positions, output_normal=self.material.requires_normal, get_new_bbox=(true_global_step % 200 == 0)
            )
            rgb_fg_all = self.material(
                viewdirs=t_dirs,
                positions=positions,
